...
class DimcArrayUnit(ImcUnit):
    ...
    # get adder depth structure list:
    # given a bitline size, find the number of adder trees required as a sum of power of 2s
    # returns a list of power of 2s
    @staticmethod
    ...
    def get_delay(self):
        ...
        self.delay_breakdown = {
            "mults":    dly_mults,
            "adders":   max(dly_adders),
            "adders_pv":max(dly_adders_pv),
            "accumulators": max(dly_accumulators),
    ...
    def get_adder_trees_energy(self, layer, logic_unit, mapped_rows_for_adder, bl_dim_size, mapped_cols, layer_act_operand_pres, macro_activation_times):
        ...
        adder_structure = self.get_adder_depth_structure(bl_dim_size)
        mapped_inputs = mapped_rows_for_adder  # number of used inputs for an adder tree
        fully_activated_number_of_1b_adder, half_activated_number_of_1b_adder = 0, 0
        mapped_inputs_current = 0

        for ii_nb, nb_inputs_of_adder in enumerate(adder_structure):
            adder_depth = math.log2(nb_inputs_of_adder)
            ...
            if mapped_inputs + min([0, ii_nb]) <= nb_inputs_of_adder:
                mapped_inputs_current = mapped_inputs + min([0, ii_nb])
            else:
                mapped_inputs_current = nb_inputs_of_adder
                    
            mapped_inputs_current = mapped_inputs if mapped_inputs <= nb_inputs_of_adder else nb_inputs_of_adder
            ...
            if mapped_inputs_current >= 1:
                if mapped_inputs_current == nb_inputs_of_adder:
                    """
...
#                    half_activated_number_of_1b_adder = 0
                    left_input = mapped_inputs_current
                    ...
                    while left_input != 0:
                        ...
                        if left_input <= 1 and baseline == 1:  # special case
                            ...
                            half_activated_number_of_1b_adder += adder_input_pres + activated_depth
                            left_input = left_input - baseline
                mapped_inputs -= nb_inputs_of_adder
