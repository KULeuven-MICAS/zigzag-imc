...
class SpatialMappingMixConversionStage(Stage):
    ...
    def run(self):
        ...
        # TODO: [jiacong] [ADD] update user_spatial_mapping if it is changed
        updated_user_spatial_mapping = {}
        for oa_dim, sm_loop in user_spatial_mapping.items():
            if isinstance(sm_loop[0], str):
                ...
                sm_comb = []
                for sm_ele in sm_loop:
                    sm_layer_dim = sm_ele[0]
                    for sm_element in spatial_mapping.spatial_loop_dim_size:
                        if sm_element[0] == sm_layer_dim:
        ...
        logger.info(
            f"{updated_user_spatial_mapping}:"
        )
    ...
    def generate_limited_user_spatial_mapping(self, layer_dim_sizes, oa_dims, oa_dim_name, spatial_loop):
    ...
    def convert_user_spatial_mapping(self, user_spatial_mapping):
        ...
        # TODO: [jiacong] [MODIFY] to support mix spatial_loop combination
        for spatial_comb in user_spatial_mapping.items():
            (oa_dim_name, spatial_loop) = spatial_comb
            if not isinstance(spatial_loop[0], tuple): # single layer_dim unrolling

                limited_user_spatial_mapping_to_check = self.generate_limited_user_spatial_mapping(layer_dim_sizes, oa_dims, oa_dim_name, spatial_loop)
                if limited_user_spatial_mapping_to_check == None:
                    ...
                    limited_user_spatial_mapping[oa_dim_name] = limited_user_spatial_mapping_to_check
            else: # mix layer_dim unrolling
                limited_user_spatial_mapping[oa_dim_name] = []
                for spatial_loop_element in spatial_loop:
                    limited_user_spatial_mapping_to_check = self.generate_limited_user_spatial_mapping(layer_dim_sizes,
                                                                                                       oa_dims,
                                                                                                       oa_dim_name,
                    ...
                    if limited_user_spatial_mapping_to_check == None:
                        ...
                        limited_user_spatial_mapping[oa_dim_name].append(limited_user_spatial_mapping_to_check)
                limited_user_spatial_mapping[oa_dim_name] = tuple(limited_user_spatial_mapping[oa_dim_name])
        # TODO: [jiacong] [FINISH]
            # Jiacong, 9/12/2023: The block below seems no use. Commented out.
            #######################################
            ...
            for memory_level in memory_levels:
                ...
                for dimension in served_dimensions:
                    ...
                    if dim_name in user_sm_copy:
                        ...
                        # TODO: [jiacong] [MODIFY] reformat the spatial_loop to the original format
                        if not isinstance(spatial_loop[0], tuple): # single layer_dim unrolling: it is still original format
                            spatial_mapping_lvl.append(spatial_loop)
                        else: # mix comb: reformat and remove the outer nested tuple
                            for spatial_mapping_element in spatial_loop:
                                spatial_mapping_lvl.append(spatial_mapping_element)
                        # TODO: [jiacong] [FINISH]
