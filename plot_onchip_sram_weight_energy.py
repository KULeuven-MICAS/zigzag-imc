...
def get_df(directory):
    ...
    for filename in os.listdir(directory):
        ...
        for l in data:
            ...
            print(l.mem_area_breakdown['weight_sram'])
            df_data.append({'layer':l.layer.id,
                'loop_dim_size':l.layer.loop_dim_size,
                'area':l.imc_area + l.mem_area_breakdown['weight_sram'],
                ...
                'energy_total': sum([sum(v[:-1]) for k,v in l.mem_energy_breakdown.items()] + [x for x in l.MAC_energy_breakdown.values()]),
...
#    df = df.sort_values(by=['area','latency_total'],ascending=[True,True],ignore_index=True).drop_duplicates(['layer','latency_total'])
    ...
    fig.show()
    with open("onchip_weights_resnet8_energy.pkl","wb") as infile:
