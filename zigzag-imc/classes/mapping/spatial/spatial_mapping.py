...
class SpatialMapping:
    ...
    def calc_unit_count(self):
        ...
        # round 2 times, in avoid of a number being x.50000000000001 rounded to x+1
        unit_count = {op: [
            round(round(prod(self.unroll_size_total[op][lv:self.arch_level[op]]),2)) for lv in range(self.arch_level[op])
    ...
    def calc_mem_bw_boost_factor(self):
        ...
        mem_bw_boost = {op: [
            self.unit_unique[op][lv]/self.unit_unique[op][lv+1] for lv in range(self.arch_level[op]-1)
        ] for op in self.operand_list}
