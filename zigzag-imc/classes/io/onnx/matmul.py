...
from classes.io.onnx.parser import Parser
...
class MatMulParser(Parser):
    ...
    def generate_layer_node_for_matmul(self):
        ...
        def get_layer_node_input_format(B, C, K, node_mapping, nodes_outputs):
            ...
            operand_conv = {'W':'B', 'I':'A', 'O':'O','O_final':'O_final'}

            d["dimension_relations"] = []
            d["operand_precision"] =  {}
            for k,v in node_mapping['operand_precision'].items():
                d['operand_precision'][operand_conv[k]] = v #{'O': 16, 'O_final': 8, 'W': 8, 'I': 8}
            ...
            elif 'spatial_mapping_hint' in node_mapping.keys():
                d["spatial_mapping_hint"] =  node_mapping["spatial_mapping_hint"]
            else:
                raise Exception("No spatial mapping provided in the mapping settings")
            if 'temporal_ordering' in node_mapping.keys():
                d['temporal_ordering'] = node_mapping['temporal_ordering']



            d["memory_operand_links"] =  {'O': 'O', 'B': 'I1', 'A': 'I2'}
