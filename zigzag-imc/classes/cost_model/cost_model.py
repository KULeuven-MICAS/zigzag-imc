...
def calc_MUW_union(port_duty_list):
    ...
    for port_duty in port_duty_list:
        ...
        input_dict[key] = {'P': round(port_duty.period), 'A': round(port_duty.allowed_cycle), 'PC': round(port_duty.period_count)}
    ...
    indicators = np.zeros((len(input_dict), int(ceil(max_period))), dtype=np.int8)
    for i, op in enumerate(input_dict):
        ''' reshape to period of this operand '''
        try:
            indicators_reshape = indicators.reshape((len(input_dict), -1, int(input_dict[op]['P'])))
        except:
            breakpoint()
        ''' fill in first few time units as used '''
        indicators_reshape[i, :, :int(input_dict[op]['A'])] = 1
...
class CostModelEvaluation:
    ...
    def __init__(self, *, accelerator, layer, spatial_mapping, temporal_mapping):
        ...
        * mapping: The combined spatial and temporal mapping object where access patterns are computed.
        ...
        self.mem_r_bw_dict_imc = copy.deepcopy(self.mem_r_bw_dict)
        self.mem_w_bw_dict_imc = copy.deepcopy(self.mem_w_bw_dict)
        self.mem_r_bw_min_dict_imc = copy.deepcopy(self.mem_r_bw_min_dict)
        if 'W' in self.mapping.spatial_mapping.mapping_dict_origin.keys():
            num_cols = np.prod([x[1] for x in self.mapping.spatial_mapping.mapping_dict_origin['W'][1] if x[0] in ['OX','K']])
        else:
            num_cols = np.prod([x[1] for x in self.mapping.spatial_mapping.mapping_dict_origin['B'][1] if x[0] in ['OX','K']])

        op_precision = layer.operand_precision[layer.layer_attrs['constant_operands'][0]]  
        imc_bw_req = num_cols * op_precision
        try:
            self.mem_r_bw_dict_imc['I1'][1] = min(imc_bw_req, self.mem_r_bw_dict_imc['I1'][1])
            self.mem_r_bw_min_dict_imc['I1'][1] = min(imc_bw_req, self.mem_r_bw_min_dict_imc['I1'][1])
    ...
    def calc_memory_utilization(self):
        ...
        for layer_op in self.layer.operand_list:
            ...
            for mem_lv in range(self.active_mem_level[layer_op]):
                ...
                if not(layer_op == 'I' and mem_lv == 0):
                    try:
        ...
        for mem_share_dict in self.mem_sharing_list:
            ...
            # jsun: 2023/04/02 avoid error occurs at: DIMC type, workload: ds-cnn, D1: 168, D2: 32, D3:2, L2_sram: 23296 size/128 bw, in/output_buffer: 8 size/8 bw
            # jsun: 2023/05/09: remove it temporarily. It might not be correct.
            #if mem_utilization > 1:
            #    mem_utilization = 1
        ...
        for layer_op in self.layer.operand_list:
            ...
            for mem_lv in range(self.mapping.mem_level[layer_op]):
                ...
                if data_elem_move_per_period == 0 or data_precision == 0:
                    ...
                    ''' 29/06/2023 update the way of calculation for a correct energy calculation when max_bw<another_side_bw '''
                    wr_in_by_low = (data_elem_move_per_period * data_precision) / max_bw * \
                                   total_period_count * \
                                   self.mapping.spatial_mapping.unit_count[layer_op][mem_lv + 1]
                    #wr_in_by_low = ceil((data_elem_move_per_period * data_precision) / min_bw) * \
                    #               (min_bw / max_bw) * \
                    #               total_period_count * \
                    #               self.mapping.spatial_mapping.unit_count[layer_op][mem_lv + 1]
                ...
                if data_elem_move_per_period == 0 or data_precision == 0:
                    ...
                    if mem_lv > 0:
                        ...
                        cycle_in_a_period = 1
                    ''' 29/06/2023 update the way of calculation for a correct energy calculation when max_bw<another_side_bw '''
                    rd_out_to_low = (data_elem_move_per_period * data_precision) / max_bw * \
                                    total_period_count * \
                                    self.mapping.spatial_mapping.unit_count[layer_op][mem_lv + 1]
                ...
                if data_elem_move_per_period == 0:
                    ...
                    max_bw = self.mem_r_bw_dict[self.layer_op_to_mem_op[layer_op]][mem_lv]
                    min_bw = self.mem_r_bw_min_dict[self.layer_op_to_mem_op[layer_op]][mem_lv]
                    rd_out_to_high = (data_elem_move_per_period * data_precision) / max_bw * \
                                     total_period_count * \
                                     self.mapping.spatial_mapping.unit_count[layer_op][mem_lv + 1]
                ...
                if data_elem_move_per_period == 0:
                    ...
                    max_bw = self.mem_w_bw_dict[self.layer_op_to_mem_op[layer_op]][mem_lv]
                    min_bw = self.mem_w_bw_min_dict[self.layer_op_to_mem_op[layer_op]][mem_lv]
                    wr_in_by_high = (data_elem_move_per_period * data_precision) / max_bw * \
                                    total_period_count * \
                                    self.mapping.spatial_mapping.unit_count[layer_op][mem_lv + 1]
    ...
    def calc_MAC_energy_cost(self):
        ...
        self.MAC_energy_breakdown,self.imc_cells_write = core.get_mac_energy(self.layer, self.mapping)
    ...
    def calc_memory_energy_cost(self):
        ...
        for (layer_op, mem_access_list_per_op) in self.memory_word_access.items():
            ...
            for (access_count, memory_level) in zip(mem_access_list_per_op, memory_levels):
                ...
                # 2023/6/8: following if condition calculation is incorrect. Commented.
                #if mem_op == 'I1' and breakdown == []:
                #    write_in_energy_from_above = self.imc_cells_write * energy_cost_per_write_in * self.mapping_int.unit_mem_data_movement[layer_op][0].data_trans_period_count.wr_in_by_high #access_count.get_total_write_ins_from_above(scaling=energy_cost_per_write_in)
                #else:
                #    write_in_energy_from_above = access_count.get_total_write_ins_from_above(scaling=energy_cost_per_write_in)
    ...
    def calc_allowed_and_real_data_transfer_cycle_per_DTL(self):
        ...
        for layer_op in self.layer.operand_list:
            ...
            for mem_lv in range(self.mapping_int.mem_level[layer_op]):
                ...
                mem_bw = self.mem_w_bw_dict_imc[mem_op][mem_lv] * self.mac_clock_domain
                ...
                mem_bw = self.mem_r_bw_dict_imc[mem_op][mem_lv] * self.mac_clock_domain
    ...
    def combine_data_transfer_rate_per_physical_port(self):
        ...
        if self.accelerator.cores[0].operational_array.type in ['AIMC','DIMC']:
            '''2023/7/5 support when D3>1 in imc array'''
            try:
                sm_d1 = self.mapping.spatial_mapping.mapping_dict_origin['I'][0] # mapping on dimension D1
            except:
                sm_d1 = self.mapping.spatial_mapping.mapping_dict_origin['A'][0]
            sm_d2 = self.mapping.spatial_mapping.mapping_dict_origin['O'][0] # mapping on dimension D2
            sm_d1 = [i for i in sm_d1]
            sm_d2 = [i for i in sm_d2]
            Ku = np.prod([x[1] for x in sm_d1 if x[0] == 'K'])
            FXu = np.prod([x[1] for x in sm_d2 if x[0] == 'FX'])
            FYu = np.prod([x[1] for x in sm_d2 if x[0] == 'FY'])
            Cu = np.prod([x[1] for x in sm_d2 if x[0] == 'C'])
            OXu = np.prod([x[1] for x in sm_d1 if x[0] == 'OX'])
            tm = self.mapping.temporal_mapping.mapping_dic_origin['O']
            tm = [j for i in tm for j in i]

            #sm = self.mapping.spatial_mapping.mapping_dict_origin['O']
            #tm = self.mapping.temporal_mapping.mapping_dic_origin['O']
            #sm = [j for i in sm for j in i]
            #tm = [j for i in tm for j in i]
            #total_unrollings = np.prod([x[1] for x in sm])
            #Ku = np.prod([x[1] for x in sm if x[0] == 'K'])
            #FXu = np.prod([x[1] for x in sm if x[0] == 'FX'])
            #FYu = np.prod([x[1] for x in sm if x[0] == 'FY'])
            #Cu = np.prod([x[1] for x in sm if x[0] == 'C'])
            #OXu = np.prod([x[1] for x in sm if x[0] == 'OX'])
            ''' 2023/6/20 support when m_factor>1 in imc array '''
            used_m = 1 # initilization
            try:
                m_tm_loop = self.mapping.temporal_mapping.mapping_dic_origin['W'][0] # tm on lowest 'W' mem level
                if len(self.mapping.temporal_mapping.mapping_dic_origin['W']) == 1: # dram lv is removed
                    weight_loading = False
                else:
                    weight_loading = True
            except:
                m_tm_loop = self.mapping.temporal_mapping.mapping_dic_origin['B'][0] # some layers use 'B' instead of 'W'
                if len(self.mapping.temporal_mapping.mapping_dic_origin['B']) == 1: # dram lv is removed
                    weight_loading = False
                else:
                    weight_loading = True
            for k, v in m_tm_loop:
                if k in ['FX','FY','K','C','G']: # k relavant loop
                    used_m *= v
            weight_reloads = port_activity_collect[0]['rw_port_1'][0].period_count

            num_rows = Cu * (FYu * (OXu + FXu - 1)) # mapped input channel
            weight_loading_cycles = (weight_reloads) * num_rows * used_m * weight_loading
        SS_comb_list.append(weight_loading_cycles)

        self.weight_loading_cycles = weight_loading_cycles
