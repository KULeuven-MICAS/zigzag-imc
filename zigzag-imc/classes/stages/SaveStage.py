...
class CompleteSaveBestStage(Stage):
    ...
    def __init__(self, list_of_callables, *, dump_filename_pattern, **kwargs):
        ...
        if type(dump_filename_pattern) == str:
            self.dump_filename_pattern = dump_filename_pattern
        else:
            self.dump_filename_pattern = dump_filename_pattern[1]
            self.hd = dump_filename_pattern[0]
    ...
    def run(self) -> Generator[Tuple[CostModelEvaluation, Any], None, None]:
        ...
        if self.dump_filename_pattern.split('.')[-1] == "json":
            ...
            with open(self.dump_filename_pattern,'wb') as infile:
                pickle.dump((self.hd, cme_best), infile)
                logger.info(f"Saved BEST CME with energy {best_energy}")
