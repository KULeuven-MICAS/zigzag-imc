...
def generate_zigzag_hardware_aimc(filepath, hd):
    con = """
import os
...
def multiplier_array():
    '''
    AIMC array variables
    :param multiplier_input_precision: activation precision, weight precision
    :param AIMC_unit_costs['vdd']:      supply voltage
    :param AIMC_unit_costs['CORE_ROWS']:how many rows share one input channel
    :param AIMC_unit_costs['INPUT_BITS_PER_CYCLE']: bit precision per input channel, must be 1 (or the adder tree cost and adder tree input precision is not correct)
    :param AIMC_unit_costs['WEIGHT_BITCELL']: storage precision
    :param AIMC_area: area per cell(no use), area of 1 ADC, area of 1 DAC
    ...
    AIMC_unit_costs = {{'vdd' : {vdd},
            'CORE_ROWS' : {cells_per_mult},
            'DAC_RES': {input_precision},
            'ADC_RES': {adc_resolution}, # constant to 8 for experiment
