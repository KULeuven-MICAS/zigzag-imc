...
def mp_dp(workload, accelerator_type, rc):
    ...
    dram_bw = cols * number_of_cores # unit: bit, set it equal to total nb_columns
    ...
    hash_tuple = (activation_precision, input_precision, rows, cols, m_factor, input_toggle_rate, weight_sparsity, vdd, number_of_cores, random.randbytes(1))
    #####################################
    ...
    logging.info(f'res_in_chl, res_out_chl in {[x for x in zip(res_in_chl, res_out_chl)]}, M {m_factor} number_of_cores {number_of_cores} MEM_RATIO {mem_ratio}')

    for r_in_chl, r_out_chl in zip(res_in_chl, res_out_chl):
        #if r_in_chl < 8 or r_out_chl < 8:
        #    logging.debug(f'[skip] r_in_chl: {r_in_chl}, r_out_chl: {r_out_chl}')
        ...
        if accelerator_type in ['aimc_max', 'aimc_min']:
            area, tclk, tops, topsw, topsmm2, mem_density, peak_dict = get_aimc_cost.get_aimc_cost(cacti_path, \
                ...
                unit_delay_28nm, unit_cap_28nm, vdd, max_adc_resolution)
        else:
            area, tclk, tops, topsw, topsmm2, mem_density, peak_dict = get_imc_cost.get_imc_cost(cacti_path, \
                    ...
                    weight_sparsity, unit_area_28nm, unit_delay_28nm, unit_cap_28nm, vdd)
        logging.info(f'IMC Area {area:.7f}, tclk {tclk:5.1f} topsw {topsw:5.0f}')  
        ...
        while True:
            try:
                ...
                logging.info(f'[INFO] cache setting found, size: {cache_size}-bit, bw: {cache_bw}. Area target={area_target_cache}')
                ...
                logging.info(f'current bw of cache is too high to meet the area budget and CACTI did not find any topology. bw will be halved from {cache_bw} to {cache_bw//2} and rerun CACTI.')
...
def main():
    ...
    for accelerator_type in ['dimc']:
        ...
        for workload in ['ds-cnn']:
            ...
            input_pres = [1] # input precision per channel
