...
class MultiplierArray:
    def __init__(self, vdd: float, input_precision: int, number_of_multiplier: int, number_of_mapped_multiplier: int, unit_area: float, unit_delay: float, unit_cap: float):
        """
        Class for a single multiplier that performs 1 bit x multiple bits
        :param vdd:                  The supply voltage (unit: V)
        :param input_precision:      The bit precision of the input (unit: bit)
        ...
        :param number_of_mapped_multiplier: The number of multiplier with non-zero input (weight is not taken into account)
        """
        if(number_of_mapped_multiplier > number_of_multiplier):
            raise ValueError(f"MultiplierArray: number_of_mapped_multiplier should not exceed number_of_multiplier. number_of_mapped_multiplier: {number_of_mapped_multiplier}, number_of_multiplier: {number_of_multiplier}")
        if(vdd < 0 or input_precision < 0 or number_of_multiplier < 0 or number_of_mapped_multiplier < 0 or unit_area < 0 or unit_delay < 0 or unit_cap < 0):
            raise ValueError("MultiplierArray: there should be no negtive value for parameters. Please recheck.")
        self.mult = Multiplier(vdd, input_precision, unit_area, unit_delay, unit_cap)
        self.vdd = vdd
        self.input_precision = input_precision
        self.output_precision = input_precision # output precision = input precision
        self.number_of_multiplier = number_of_multiplier
        self.number_of_mapped_multiplier = number_of_mapped_multiplier
    ...
    def calculate_energy(self):
        ...
        energy: The energy cost (unit: fJ)
        """
        energy = self.mult.calculate_energy() * self.number_of_mapped_multiplier
        return energy
...
class Adder:
    ...
    def calculate_energy(self):
        ...
        return energy
    def calculate_sum_energy(self):
        """
        sum_energy: The energy cost only for sum (each time it is triggered) (unit: fJ)
        """
        sum_energy = 2*self.xor2.calculate_cap() * self.vdd**2 * self.number_of_1b_adder
        return sum_energy
        

class AdderTree:
    def __init__(self, vdd: float, input_precision: int, number_of_input: int, number_of_mapped_input: int, unit_area: float, unit_delay: float, unit_cap: float):
        """
        Class for a {input_number} {input_precision}-b Carry-Ripple Adder Tree
        :param vdd:                The supply voltage (unit: V)
        :param input_precision:    The bit precision of the input (unit: bit)
        :param number_of_input:    The number of inputs
        :param number_of_mapped_input: The number of non-zero inputs
        :param output_precision:   The bit precision of the output (unit: bit)
        :param number_of_1b_adder: The number of 1-b adder in the adder tree
        """
        if(number_of_mapped_input > number_of_input):
            raise ValueError(f"AdderTree: number_of_mapped_input should not exceed number_of_input. number_of_mapped_input: {number_of_mapped_input}, number_of_input: {number_of_input}")
        if(math.log(number_of_input,2)%1 != 0):
            raise ValueError("AdderTree: the number of input for the adder tree is not in the power of 2. Currently it is: %s" %number_of_input)
        if(vdd < 0 or input_precision < 0 or number_of_mapped_input < 0 or number_of_input < 0 or unit_area < 0 or unit_delay < 0 or unit_cap < 0):
            raise ValueError("AdderTree: there should be no negtive value for parameters. Please recheck.")
        self.vdd = vdd
        self.input_precision = input_precision
        self.number_of_input = number_of_input
        self.number_of_mapped_input = number_of_mapped_input
...
#             area_b += single_adder.calculate_area() * math.ceil( self.number_of_input/(2**(stage_idx+1)) )
        ...
        area = self.number_of_1b_adder * self.unit_adder.calculate_area()
        return area
    ...
    def calculate_energy(self):
        ...
        energy: The energy cost (each time it is triggered) (unit: fJ)
        """
        if(self.number_of_mapped_input == self.number_of_input):
            fully_activated_number_of_1b_adder = self.number_of_1b_adder
            half_activated_number_of_1b_adder = 0
        else:
            fully_activated_number_of_1b_adder = 0
            half_activated_number_of_1b_adder = 0
            left_input = self.number_of_mapped_input
            baseline = self.number_of_input
            while(left_input != 0):
                baseline = baseline/2
                activated_depth = int( math.log(baseline, 2) )
                if(left_input == 1 and baseline == 1):
                    fully_activated_number_of_1b_adder += 0
                    half_activated_number_of_1b_adder += self.input_precision
                    left_input = 0
                elif(left_input > baseline):
                    fully_activated_number_of_1b_adder += baseline*(self.input_precision+1)-(self.input_precision+activated_depth+1) + (self.input_precision + activated_depth)
                    half_activated_number_of_1b_adder += 0
                    left_input = left_input - baseline
                elif(left_input < baseline):
                    half_activated_number_of_1b_adder += self.input_precision + activated_depth
                else: # left_input == baseline
                    fully_activated_number_of_1b_adder += baseline*(self.input_precision+1)-(self.input_precision+activated_depth+1)
                    half_activated_number_of_1b_adder += self.input_precision + activated_depth
                    left_input = left_input - baseline
        energy = fully_activated_number_of_1b_adder * self.unit_adder.calculate_energy() + half_activated_number_of_1b_adder * self.unit_adder.calculate_sum_energy()
        return energy


## from zigzag hereafter
