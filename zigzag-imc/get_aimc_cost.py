from aimc_definition import *
from get_cacti_cost import *
import random
import math
import pdb

def get_aimc_cost(cacti_path, activation_precision, input_precision, weight_precision, output_precision, input_channel, output_channel, rows, input_toggle_rate, weight_sparsity, unit_area_28nm, unit_delay_28nm, unit_cap_28nm, vdd, max_adc_resolution, eval_level='macro', cores=1, ini_hash=0):
    '''
    get area cost for a specific aimc setting (include regs_input and regs_output)
    ...
    :param max_adc_resolution:      allowed maximum adc resolution
...
############################# IMC MACRO #######################
    ...
    if input_precision < 1 or input_channel < 1 or int(input_precision) != input_precision or int(input_channel) != input_channel:
        raise Exception(f'[get_aimc_cost] [error]: input_precision: {input_precision}, input_channel: {input_channel}')

    if max_adc_resolution == 'dynamic':
        adc_resolution = 0.5*math.log2(input_channel) + input_precision 
    else:
        adc_resolution = input_precision
    dac_resolution = input_precision
    
    hash_tuple = (activation_precision, input_precision, input_channel, output_channel, rows, input_toggle_rate, weight_sparsity, vdd, max_adc_resolution, cores, ini_hash, random.randbytes(1))
    ...
    aimc_macro = AimcMacro(activation_precision, weight_precision, input_precision, input_channel, output_channel, input_channel, output_channel, rows, cols, w_bw, sram_access_time, sram_w_cost, sram_area, unit_area_28nm, unit_delay_28nm, unit_cap_28nm, adc_resolution, dac_resolution, vdd)
    ...
    tclk = max(aimc_macro.calculate_computing_delay(), aimc_macro.calculate_w_delay()) # ns

    number_of_ops = 2 * aimc_macro.calculate_number_of_macs() * cores
    ...
    if eval_level == 'system': # used for peak system performance evaluation
        ...
        energy = (aimc_macro.calculate_computing_energy(input_toggle_rate, weight_sparsity) + (regs_input.w_cost + regs_output.w_cost + regs_in_cache + regs_out_cache)/(activation_precision/input_precision)) * cores #fJ 
        total_area = (aimc_macro.calculate_area() + regs_input.area + regs_output.area) * cores + cache_area
    else:
        cache_area = 0
        regs_in_cache = 0
        regs_out_cache = 0
        energy = aimc_macro.calculate_computing_energy(input_toggle_rate, weight_sparsity) * cores #fJ
        total_area = aimc_macro.calculate_area() * cores
    ...
    peak_dict = {
            ...
            'area_breakdown_bank (mm2)':    aimc_macro.area_bank,
            'area_breakdown_mults (mm2)':   aimc_macro.area_mults,
            'area_breakdown_adc (mm2)':     aimc_macro.area_adcs,
            'area_breakdown_dac (mm2)':     aimc_macro.area_dacs,
            'area_breakdown_adder_tree (mm2)':   aimc_macro.area_adder_tree,
            'area_breakdown_accumulator (mm2)':   aimc_macro.area_accumulator,
            'area_breakdown_regs_accumulator (mm2)':   aimc_macro.area_regs_accumulator,
            'area_macros (mm2)'         :   aimc_macro.calculate_area() * cores,
            'area_cache (mm2)'          :   cache_area,
            'area_input_regs (mm2)'     :   regs_input.area*cores,
            'area_output_regs (mm2)'    :   regs_output.area*cores,
            'tclk_breakdown_cells (ns)' :   aimc_macro.array_delay,
            'tclk_breakdown_mults (ns)' :   aimc_macro.mults.calculate_delay(),
            'tclk_breakdown_dac (ns)' :   aimc_macro.dac.calculate_delay(),
            'tclk_breakdown_adc (ns)' :   aimc_macro.adc.calculate_delay(),
            'tclk_breakdown_adder_tree (ns)': aimc_macro.place_value_adder_tree.calculate_delay(),
            'tclk_breakdown_accumulator (ns)':aimc_macro.accumulator_delay,
            'imc_weight_update_delay (ns)'  : aimc_macro.calculate_w_delay(),
            ...
            'peak_energy_breakdown_bank (pJ)':      aimc_macro.energy_bank/1000,
            'peak_energy_breakdown_mults (pJ)'  :   aimc_macro.energy_mults/1000,
            'peak_energy_breakdown_dacs (pJ)'  :   aimc_macro.energy_dacs/1000,
            'peak_energy_breakdown_adcs (pJ)'  :   aimc_macro.energy_adcs/1000,
            'peak_energy_breakdown_adder_tree (pJ)':aimc_macro.energy_adder_tree/1000,
            'peak_energy_breakdown_accumulator (pJ)': aimc_macro.energy_accumulator/1000,
            'peak_energy_breakdown_regs_accumulator (pJ)': aimc_macro.energy_regs_accumulator/1000,
            'peak_energy_mac (fJ/MAC)':                     energy/number_of_ops*2,
            'peak_energy_breakdown_bank_mac (fJ/MAC)':      aimc_macro.energy_bank/number_of_ops*2,
            'peak_energy_breakdown_mults_mac (fJ/MAC)'  :   aimc_macro.energy_mults/number_of_ops*2,
            'peak_energy_breakdown_dacs_mac (fJ/MAC)'  :   aimc_macro.energy_dacs/number_of_ops*2,
            'peak_energy_breakdown_adcs_mac (fJ/MAC)'  :   aimc_macro.energy_adcs/number_of_ops*2,
            'peak_energy_breakdown_adder_tree_mac (fJ/MAC)':aimc_macro.energy_adder_tree/number_of_ops*2,
            'peak_energy_breakdown_accumulator_mac (fJ/MAC)': aimc_macro.energy_accumulator/number_of_ops*2,
            'peak_energy_breakdown_regs_accumulator_mac (fJ/MAC)': aimc_macro.energy_regs_accumulator/number_of_ops*2,
    ...
    ## print mm2/MAC
    #print(total_area)
    #print(peak_dict['area_breakdown_mults (mm2)']/number_of_ops*2, peak_dict['area_breakdown_adder_tree (mm2)']/number_of_ops*2, peak_dict['area_breakdown_bank (mm2)']/number_of_ops*2, peak_dict['area_breakdown_adc (mm2)']/number_of_ops*2)
    #print(peak_dict['tclk_breakdown_mults (ns)'], peak_dict['tclk_breakdown_adder_tree (ns)'],peak_dict['tclk_breakdown_cells (ns)'], peak_dict['tclk_breakdown_adc (ns)'])
    #print(peak_dict['peak_energy_mac (fJ/MAC)'])
    #print(peak_dict['peak_energy_breakdown_mults_mac (fJ/MAC)'], peak_dict['peak_energy_breakdown_adder_tree_mac (fJ/MAC)'], peak_dict['peak_energy_breakdown_bank_mac (fJ/MAC)'], peak_dict['peak_energy_breakdown_adcs_mac (fJ/MAC)'], peak_dict['peak_energy_breakdown_dacs_mac (fJ/MAC)'],peak_dict['peak_energy_breakdown_accumulator_mac (fJ/MAC)']+peak_dict['peak_energy_breakdown_regs_accumulator_mac (fJ/MAC)'])
    #print(peak_dict['peak_energy_breakdown_input_regs (pJ)']*1000/number_of_ops*2, peak_dict['peak_energy_breakdown_output_regs (pJ)']*1000/number_of_ops*2, peak_dict['peak_energy_breakdown_cache_2_input_regs (pJ)']*1000/number_of_ops*2, peak_dict['peak_energy_breakdown_output_regs_2_cache (pJ)']*1000/number_of_ops*2)
    #print(sram_r_cost)
    return total_area, tclk, tops, topsw, topsmm2, mem_density, peak_dict

def get_aimc_config_w_area_budget(cacti_path, activation_precision, input_precision, weight_precision, output_precision, cells_per_mult, number_of_cores, area_target, unit_area_28nm, unit_delay_28nm, unit_cap_28nm, vdd, max_adc_resolution):
    '''
    search for maximum allowed aimc setting for given area budget
    Note: same module with dimc, except replacing the child-module from dimc to aimc (get_aimc_cost).
    :param cacti_path:              path where to run cacti
    :param activation_precision:    activation bit precision (unit: bit)
    :param  input_precision:        how many bits per input channel (unit: bit)
    :param weight_precision:        weight bit precision (unit: bit)
    ...
    while True:
        ...
        min_area = get_aimc_cost(cacti_path, activation_precision, input_precision, weight_precision, output_precision, input_channel, output_channel, rows, 1, 0,  unit_area_28nm, unit_delay_28nm, unit_cap_28nm, vdd, max_adc_resolution)[0]
        if min_area > area_target/number_of_cores: # if min_area is still bigger, stop searching
            status = 'stop'
            break
        '''
        ...
        while True:
            ...
            try:
                left_y = get_aimc_cost(cacti_path, activation_precision, input_precision, weight_precision, output_precision, input_channel, output_channel, rows, 1, 0,  unit_area_28nm, unit_delay_28nm, unit_cap_28nm, vdd, max_adc_resolution)[0]
            except:
            ...
            rows = input_channel * cells_per_mult
            right_y = get_aimc_cost(cacti_path, activation_precision, input_precision, weight_precision, output_precision, input_channel, output_channel, rows, 1, 0, unit_area_28nm, unit_delay_28nm, unit_cap_28nm, vdd, max_adc_resolution)[0]
        ...
        while True:
            ...
            rows = input_channel * cells_per_mult
            mid_y = get_aimc_cost(cacti_path, activation_precision, input_precision, weight_precision, output_precision, input_channel, output_channel, rows, 1, 0, unit_area_28nm, unit_delay_28nm, unit_cap_28nm, vdd, max_adc_resolution)[0]
            if right_x - left_x > 1:
                ...
                if mid_y == area_target/number_of_cores:
                    result_input_channel.append(int(2**right_x))
                    result_output_channel.append(output_channel)
                    break
                else:
                    result_input_channel.append(int(2**left_x))
                    result_output_channel.append(output_channel)
                    break
...
if __name__ == '__main__':
    ...
    # setting (input precision: DAC precision)
    activation_precision, input_precision = 8,2
    ...
    for ii_a, input_precision in enumerate([1,2,4,8]):
        ...
        topsmm2_list = []
        tclk_list = []
        for input_channel in [32,64,128,256,512,1024]:
            cells_per_mult = 1
            output_channel = input_channel/8
            rows = input_channel * cells_per_mult
            ...
            if mem_size*8/cache_bw<32: # not meet CACTI requirement
                continue
            total_area, tclk, tops, topsw, topsmm2, memd, out_dict = get_aimc_cost(cacti_path, activation_precision, input_precision, weight_precision, output_precision, input_channel, output_channel, rows, 1, 0, unit_area_28nm, unit_delay_28nm, unit_cap_28nm, vdd,max_adc_resolution, cores=core,eval_level='system')
    ...
    fig.text(0.5,0.01,'macro size (AIMC)', fontsize=15, ha='center')
